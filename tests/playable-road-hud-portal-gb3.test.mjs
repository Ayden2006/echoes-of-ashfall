...
test("late HUD/combat leftover: sword rim + HUNT/ALLY HUNT/health strokes + MAGICAL CARD FORMED cream still readable", () => {
  ...
  assert.match(game, /ctx\.fillText\(`ALLY · \$\{companionName\}\$\{huntTag\}  \$\{Math\.ceil\(ally\.health\)\} \/ \$\{ally\.maxHealth\}`,ally\.x,barY-5\); \/\/ ALLY · HUNT leftover still keeps #58 late stroke with sword rim \/ lynx \/ wyrm health; leftover still readable after #70 road-step/);
  assert.match(game, /ctx\.lineWidth=late\?4:3;ctx\.strokeStyle=late\?"rgba\(6,2,4,\.96\)":"rgba\(4,10,6,\.9\)";ctx\.strokeText\("HUNT",0,-10\); \/\/ leftover HUNT still readable after #70 road-step/);
  ...
  assert.match(game, /ctx\.lineWidth=late\?4:3;ctx\.strokeStyle=late\?"rgba\(6,2,4,\.96\)":"rgba\(10,2,10,\.9\)";ctx\.strokeText\(healthLabel,wyrm\.x\+recoilX,barY-3\); \/\/ heart wyrm health keeps late stroke with HUNT; leftover still readable after #70 road-step/);
  assert.match(game, /ctx\.lineWidth=late\?4:3;ctx\.strokeStyle=late\?"rgba\(6,2,4,\.96\)":"rgba\(20,8,4,\.9\)";ctx\.strokeText\(healthLabel,beast\.x\+recoilX,barY-3\); \/\/ kiln lynx health keeps late stroke with HUNT; leftover still readable after #70 road-step/);
