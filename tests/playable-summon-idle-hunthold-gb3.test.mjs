...
const qDuringRecall = (plats, width, playerX, facing, mapId) => {
  ...
  keepCreatureOnRoad(ally, plats, width, mapId);
  if (ally.y > ally.groundY) ally.y = ally.groundY;
  return { ally, summonX, summonFloor };
};
...
test("Q/1–5 summonFloor + idleSeat leftovers still clear thin stones after hunthold", () => {
  assert.match(game, /if\(startedRef\.current&&\/\^\[1-5\]\$\/\.test\(k\)&&!e\.repeat\)\{selectUsableSlot\(Number\(k\)-1\);return;\} \/\/ leftover 1–5 still selects during recall after #63; leftover still selects after hunthold/);
  assert.match(game, /if\(!worldMapOpenRef\.current\)deployQueued\.current=true/);
  assert.match(game, /const summonFloor=plantedFloorAt\(map,summonX\); \/\/ Tab\/1–5\/Q deploy still plants off leftover stones after portal reseat; Q-during-recall leftover after #63 still clears thin stones; leftover still clears close leftover stones after #72 road-step; leftover still clears thin stones after hunthold/);
  assert.match(game, /const summonFloor=plantedFloorAt\(map,summonX\); \/\/ Tab\/1–5\/Q deploy still plants off leftover stones after portal reseat; leftover 1–5\/Q after #63 still clears thin stones; leftover still clears close leftover stones after #72 road-step; leftover still clears thin stones after hunthold/);
  ...
  assert.match(game, /const idleSeat=!hunting\?plantedFloorAt\(map,ally\.x\):null; \/\/ leftover idle still plants after #72 road-step; close leftover stones stay STEP-only; leftover idle still plants after followHold; leftover still clears thin stones after hunthold/);
  assert.match(game, /if\(idleSeat\)\{ally\.x=creatureEdgeAt\(map,idleSeat\.x\);ally\.groundY=idleSeat\.groundY;\}/);
  assert.match(game, /ally\.y\+=\(ally\.groundY-leftover-ally\.y\)\*\(1-Math\.exp\(-12\*dt\)\); \/\/ hop\/roost leftover still eases through idle after sleep→wake and portal reseat; leftover still plants after #72 road-step; leftover still plants after followHold; leftover still plants after hunthold/);
  ...
  assert.equal(num(/const COMPANION_DEPLOY_DISTANCE = (\d+)/, "COMPANION_DEPLOY_DISTANCE"), 285);
  ...
  for (const stone of closeStones) {
    ...
    assertPlanted(stone.map, idle, `${stone.label} idleSeat after hunthold`);
    assert.notEqual(idle.x, mid, `${stone.label} idleSeat still slides off the leftover stone after hunthold`);

    for (const facing of [1, -1]) {
      const summoned = qDuringRecall(map.plats, map.w, mid, facing, stone.map);
      assert.equal(summoned.ally.recallStarted, 0, `${stone.label} Q-during-recall facing ${facing} clears recall after hunthold`);
      assertPlanted(stone.map, summoned.ally, `${stone.label} summonFloor facing ${facing} after hunthold`);
      if (standingInsideStone(map.plats, summoned.summonX, surfaceAt(map.plats, summoned.summonX) ?? stone.road)) {
        assert.notEqual(summoned.ally.x, summoned.summonX, `${stone.label} summonFloor still plants off the leftover stone after hunthold`);
      }
    }
  }
  ...
  for (const id of [1, 2, 3, 4, 5, 6]) {
    ...
    for (const stone of map.plats.filter((p) => p.h <= 24)) {
      ...
      const idle = idleSeatAt(map.plats, map.w, mid, id);
      assertPlanted(id, idle, `map ${id} thin stone ${stone.x} idleSeat after hunthold`);
      for (const facing of [1, -1]) {
        const summoned = qDuringRecall(map.plats, map.w, mid, facing, id);
        assert.equal(standingInsideStone(map.plats, summoned.ally.x, summoned.ally.groundY), false, `map ${id} summonFloor from stone ${stone.x} facing ${facing} still clears after hunthold`);
        assert.equal(cardBlockedAt(map.plats, id, summoned.ally.x), false, `map ${id} summonFloor from stone ${stone.x} stays off walls after hunthold`);
      }
    }
  }
});

test("softlock leftover after hunthold: void recover + portal heal/flash + altar E + followHold/teleport + plantedHuntHoldAt still fire on maps 1–6", () => {
  assert.match(game, /if\(pl\.y>WORLD_H\+80\)\{const floor=plantedFloorAt\(map,Math\.max\(120,pl\.x-180\)\);pl\.x=floor\.x;pl\.y=plantedYAt\(map,floor\.x\);pl\.vy=0;pl\.grounded=true;pl\.jumpsLeft=2;pl\.crouched=false;pl\.sliding=false;slideUntil\.current=0;\} \/\/ void recover still plants after #60\/#61 companionIdleLeftover; maps 1–6 leftover still fire after #66 6460 vein skip; leftover still fires after #72 road-step; leftover still fires after hunt teleport; leftover still fires after hunthold/);
  assert.match(game, /else if\(map===6&&atHeartAltar\(x\)\)\{ \/\/ altar E still wins after #56 Dell\/Rowan walk-out and #64 afterCapture trim; no nearby talk radius covers this window; leftover still fires after #66 scenery vein move; leftover still fires after hunt teleport; leftover still fires after hunthold/);
  assert.match(game, /else if\(map===6&&atHeartAltar\(pl\.x\)\)action=campaignEndedRef\.current\?"Rest at Ashfall's Heart":"Press E at Ashfall's Heart"; \/\/ altar prompt still wins after #56 Dell\/Rowan walk-out and #64 afterCapture trim; leftover still fires after #66 scenery vein move; leftover still fires after hunt teleport; leftover still fires after hunthold/);
  assert.match(game, /const followHold=!hunting\?plantedFloorAt\(map,followX\):null; \/\/ leftover follow still plants after #72 road-step; close leftover stones stay off allies; leftover still plants after followHold; hunt path still uses hunted\.x; leftover still plants after hunt teleport; leftover still plants after hunthold/);
  assert.match(game, /const seat=followHold\?\?plantedFloorAt\(map,followX\); \/\/ leftover teleport still plants after followHold; hunt path still uses hunted\.x; leftover still plants after hunt teleport; leftover still plants after hunthold/);
  assert.match(game, /const huntHold=hunting\?plantedHuntHoldAt\(map,rawHoldX,targetX\):null; \/\/ leftover hunt hold still plants after hunt teleport; strike range still holds; leftover stones stay STEP-only; leftover still plants after hunthold/);
  assert.match(game, /return bestInRange\?\?best\?\?plantedFloorAt\(map,rawHoldX\); \/\/ leftover hunt hold still plants after hunt teleport; strike range still holds; leftover stones stay STEP-only; leftover still plants after hunthold/);
  ...
  assert.ok(enterMap, "enterMap callback should stay intact after hunthold");
  assert.match(enterMap[0], /pl\.health=pl\.maxHealth;staminaRef\.current=MAX_STAMINA/);
  assert.match(enterMap[0], /setHealth\(pl\.maxHealth\);setStamina\(MAX_STAMINA\); \/\/ portal heal still fires after companion reseat; leftover still fires after #70 groundAt signature; leftover still fires after #72 road-step; leftover still fires after hunt teleport; leftover still fires after hunthold/);
  assert.match(enterMap[0], /const arrivalGround=seat\.groundY; \/\/ companion portal reseat still plants after #38 floors; leftover still plants after #70 groundAt signature; leftover still plants after #72 road-step; leftover still plants after hunt teleport; leftover still plants after hunthold/);
  assert.match(enterMap[0], /portalFlashUntil\.current=performance\.now\(\)\+430; \/\/ portal flash still fires after companion reseat; leftover still fires after #70 groundAt signature; leftover still fires after #72 road-step; leftover still fires after hunt teleport; leftover still fires after hunthold/);
  assert.match(enterMap[0], /tone\(610,\.25,\.028\);window\.setTimeout\(\(\)=>tone\(360,\.2,\.02\),100\); \/\/ portal enter tone still fires after companion reseat; leftover still fires after #70 groundAt signature; leftover still fires after #72 road-step; leftover still fires after hunt teleport; leftover still fires after hunthold/);
  ...
  assert.ok(healIdx >= 0 && reseatIdx > healIdx && flashIdx > reseatIdx, "heal, planted reseat, then flash still run in that order after hunthold");
  ...
  for (const layout of layouts) {
    ...
    assert.deepEqual(walkableGaps(map.plats, map.w), [], `map ${layout.id} stays gapless for leftover void recover after hunthold`);
    for (const x of layout.voids) {
      const recovered = recoverFromVoid(map.plats, map.w, x, layout.id);
      assertPlanted(layout.id, { x: recovered.x, y: recovered.groundY, groundY: recovered.groundY }, `map ${layout.id} void recover x=${x} after hunthold`);
      ...
      assertPlanted(layout.id, follow.ally, `map ${layout.id} followHold after void recover x=${x} after hunthold`);

      const teleport = huntTeleportAt(map.plats, map.w, recovered.x, 1, recovered.x + 80, layout.id);
      assertPlanted(layout.id, teleport.ally, `map ${layout.id} hunt teleport after void recover x=${x} after hunthold`);

      const hold = huntHoldAt(map.plats, map.w, recovered.x - 400, recovered.x + 80, layout.id);
      assertPlanted(layout.id, hold.ally, `map ${layout.id} plantedHuntHoldAt after void recover x=${x}`);
    }
    ...
    assertPlanted(layout.id, { x: enter.player.x, y: enter.player.groundY, groundY: enter.player.groundY }, `map ${layout.id} portal enter plant after hunthold`);
    assertPlanted(layout.id, enter.ally, `map ${layout.id} portal companion reseat after hunthold`);
  }

  assert.equal(shouldTeleport(2680, 2680 - 800, 2680), true, "ally stuck west of a perch still teleports into the hunt after hunthold");
  assert.equal(shouldTeleport(2680, 2600, 2680), false, "ally already on the hunt leash does not teleport after hunthold");
  assert.equal(shouldTeleport(400, 1300, 1800), true, "far follow without a nearby hunt still teleports after hunthold");
  assert.equal(shouldTeleport(2140, 2140 - 900, 4520), true, "map 5 kiln-road ally stuck off the lynx still teleports after hunthold");
  assert.equal(shouldTeleport(2480, 2480 - 860, 4400), true, "map 6 heart-road ally stuck off the pulse still teleports after hunthold");
  ...
  for (const x of [heartX, altarX, rim, ...recoverXs]) {
    assert.equal(atHeartAltar(x), true, `E still reaches the heart from x=${x} after hunthold`);
    assert.notEqual(surfaceAt(maps[6].plats, x), null, `altar stand x=${x} stays on the road after hunthold`);
  }
  assert.ok(veinX + 140 < heartX - range, "studyable vein stays west of altar E after hunthold");
});

test("jump/slide + ROAD_STEP leftover still hold after hunthold; thin stones stay STEP-only; hunt hold still prefers COMPANION_STRIKE_RANGE", () => {
  assert.equal(num(/const STEP_HEIGHT = (\d+)/, "STEP_HEIGHT"), 32);
  assert.equal(num(/const ROAD_STEP_HEIGHT = (\d+)/, "ROAD_STEP_HEIGHT"), 56);
  assert.match(game, /const ROAD_STEP_HEIGHT = 56; \/\/ leftover maps 1\/3 road seams still hold walk\/slide; leftover stones keep STEP_HEIGHT; leftover maps 2\/4\/5\/6 walk\/slide still hold after #70 road-step; leftover still holds after hunt teleport; leftover still holds after hunthold/);
  ...
  assert.match(game, /if\(jump&&pl\.jumpsLeft>0\)\{\n {10}const secondJump=pl\.jumpsLeft===1;\n {10}pl\.vy=secondJump\?-465:-500;pl\.grounded=false;pl\.jumpsLeft-=1;pl\.crouched=false;pl\.sliding=false;slideUntil\.current=0;didJump=true; \/\/ Space jump\/double jump leftover still readable after road-seam hold; leftover still holds after hunthold/);
  assert.match(game, /if\(wantsSlide&&pl\.grounded&&Math\.abs\(pl\.vx\)>55\)\{ \/\/ S crouch\/slide leftover still readable after road-seam hold; leftover still holds after hunthold/);
  assert.match(game, /if\(pl\.vy>=0&&ground<Infinity&&\(oldBottom<=ground\+STEP_HEIGHT&&newBottom>=ground\|\|roadStepHold\(wasGrounded&&!didJump,oldBottom,ground\)\)\)\{pl\.y=ground-PH;pl\.vy=0;pl\.grounded=true;pl\.jumpsLeft=2;\}else\{pl\.grounded=false;pl\.crouched=false;pl\.sliding=false;slideUntil\.current=0;\} \/\/ leftover maps 1\/3 road seams still hold walk\/slide; Space jump\/double jump \+ S crouch\/slide stay; leftover maps 2\/4\/5\/6 walk\/slide still hold after #70 road-step; leftover still holds after hunt teleport; leftover still holds after hunthold/);
  assert.doesNotMatch(game, /coyoteTime|ledgeForgiv|tripleJump|jumpsLeft\s*=\s*3/);
  assert.match(game, /if\(huntDist<COMPANION_STRIKE_RANGE&&dist<bestInRangeDist\)\{bestInRange=\{x:nx,groundY:g\};bestInRangeDist=dist;\}/);
  assert.match(game, /return bestInRange\?\?best\?\?plantedFloorAt\(map,rawHoldX\)/);
  ...
  for (const seam of leftoverClimb) {
    ...
    assert.equal(groundAt(plats, edge, seam.from, false), Infinity, `map ${seam.map} leftover climb ${seam.from}→${seam.to} still needs ROAD_STEP after hunthold`);
    assert.equal(groundAt(plats, edge, seam.from, true), seam.to, `map ${seam.map} leftover road-step hold is not undone after hunthold`);
    assert.equal(roadStepHold(true, seam.from, seam.to), true);
  }

  for (const id of [1, 2, 3, 4, 5, 6]) {
    const map = maps[id];
    assert.deepEqual(walkableGaps(map.plats, map.w), [], `map ${id} leftover stays gapless after hunthold`);
    const east = walkRoad(map.plats, map.w, 230, 1);
    assert.equal(east.stuck, false, `map ${id} leftover walk east still holds after hunthold`);
    ...
    assert.equal(west.stuck, false, `map ${id} leftover walk west still holds after hunthold`);
    ...
    for (const stone of map.plats.filter((p) => p.h <= 24)) {
      ...
      assert.notEqual(stepOnto, stone.y, `map ${id} leftover stone at ${stone.x} must not steal a grounded step after hunthold`);
      assert.ok(road - stone.y > STEP_HEIGHT, `map ${id} leftover stone at ${stone.x} stays above STEP_HEIGHT after hunthold`);
    }
  }
  ...
  for (const hunt of huntPacks) {
    ...
    for (const huntedX of hunt.xs) {
      assert.notEqual(surfaceAt(map.plats, huntedX), null, `${hunt.id} x=${huntedX} still stands on the road after hunthold`);
      for (const fromLeft of [true, false]) {
        ...
        assert.equal(hold.targetX, huntedX, `${hunt.id} hunt path still uses hunted.x after hunthold`);
        assertPlanted(hunt.map, hold.ally, `${hunt.id} huntHold from ${fromLeft ? "left" : "right"} at ${huntedX} after hunthold`);
        assert.equal(canStrike(hold.holdX, huntedX), true, `${hunt.id} huntHold at ${huntedX} still prefers COMPANION_STRIKE_RANGE`);
      }
    }
  }
...
test("sword/combat leftover readability still holds after hunthold; PR #10 numbers stay", () => {
  assert.match(game, /ctx\.imageSmoothingEnabled=false;ctx\.shadowColor=late\?"rgba\(255,246,210,\.88\)":"rgba\(135,62,198,\.3\)";ctx\.shadowBlur=late\?12:7; \/\/ LMB sword rim stays readable on maps 5–6 after #48\/#50 late stroke; leftover still readable after #70 road-step; leftover still readable after hunt teleport; leftover still readable after hunthold/);
  assert.match(game, /ctx\.lineWidth=late\?4:3;ctx\.strokeStyle=late\?"rgba\(6,2,4,\.96\)":"rgba\(4,10,6,\.9\)";ctx\.strokeText\("HUNT",0,-10\); \/\/ leftover HUNT still readable after #70 road-step; leftover still readable after followHold; leftover still readable after hunt teleport; leftover still readable after hunthold/);
  assert.match(game, /ctx\.fillText\(`ALLY · \$\{companionName\}\$\{huntTag\}  \$\{Math\.ceil\(ally\.health\)\} \/ \$\{ally\.maxHealth\}`,ally\.x,barY-5\); \/\/ ALLY · HUNT leftover still keeps #58 late stroke with sword rim \/ lynx \/ wyrm health; leftover still readable after #70 road-step; leftover still readable after followHold; leftover still readable after hunt teleport; leftover still readable after hunthold/);
...
test("locks hold: Moon Night, #19–#76 helpers, PR #10 numbers, talk tables, no dating, no maps 7+", () => {
