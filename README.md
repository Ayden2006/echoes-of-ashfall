...
- Map 4 — Moonwell Cliffs: Pale Stag; Orrin stands on the west cliff; Calen can be met again by the well road; Hale keeps the quiet stretch after that watch; Lira can be met again on the later cliff; Wren can be met again on the far cliff; Ryn keeps the last gate and says it heals; a cliff notch and the moonwell can be studied with `E`; far gate opens into The Quiet Ember
- Map 5 — The Quiet Ember (kiln terrace): Ember Lynx; Reed talks if you press `E`; Isk can be met again on the early coals; Vess, Perrin, and Maer can be met again on the mid coals; Hale can be met again on the quiet stretch after that watch; Tamsin can be met again on the later coals; Ryn can be met again before the east gate; Sera can be met again near the east gate; the quiet kiln, a banked coal-bed, and quiet bellows can be studied with `E`
- Map 6 — Ashfall's Heart (inner chamber): Heart Wyrm; Kest talks if you press `E`; Bram can be met again west of the wyrm; Nia, Holt, and Rowan can be met again east of the wyrm; Dell can be met again on the last gold; Edan keeps the last stretch to the altar; a first-step stone, a cooled vein, an echo-stone, and the Ashfall Heart Altar all use `E` (the altar ends the campaign)
...
Named travelers stand on the road and talk if you press `E` — the same talk used by Reed and Kest. First words keep one tell per traveler — weather, counting, ash, writing, listening — then a later talk and talk after you bind that map's animal all change. Early and mid-road first talks quietly teach the same three beats: the animals are the echo, bind then go east, and portals heal. First talks also quietly point to one same-map studyable with press `E` — the nearest plaque, merlon, shell, post, tide-cut step, cairn, notch, lichen, coal-bed, bellows, first-step stone, or cooled vein on that traveler's map — without a finish dump or walk-out. Map 1 keeps that teaching short and does not dump the heart-altar ending. After-capture lines treat bound animals as echo shards, not quarry — rain-spark, dusk shard, leftover fire at the cairn, moonwell pool, kiln heat, then the heart pulse — without reciting the whole road at every crossing. After-capture talks also quietly point to one same-map studyable with press `E` — the nearest or already-named groove, plaque, merlon, shell, post, tide-cut step, cairn, moonwell, notch, lichen, coal-bed, bellows, first-step stone, or cooled vein — without a finish dump. Maps 5–6 after-capture keep walk-out as people and gate heals; maps 1–4 stay off walk-out. Late-map first talks name the now-actions: bind the local beast if you still need it, the east gate heals, and pressing `E` at the heart altar ends the campaign — without saying you are stuck if you skipped a mid-road bind. Meet someone again farther east and a later talk is one short beat of the last crossing — castle rain, shore dusk, cairn twist, or kiln — without repeating that finish dump. A later talk can also quietly point to one nearby studyable — rain groove or plaque, dusk-shell or drowned post, foxfire hollow or split cairn, cliff notch or moonwell, kiln or coal-bed or bellows, echo-stone or altar — with press `E`, without dumping every secret at once. Heart-road reunions — Bram, Nia, Holt, Dell, and Rowan — point to the nearer echo-stone with press `E`, and keep the last crossing instead of another finish dump. After the last pulse, Reed, Kest, Edan, Hale, Ryn, Dell, Rowan, Bram, Nia, and Holt talk about walking out as people — not a dating beat, and not another campaign dump. This is not a dating sim: there are no bond meters, romance choices, or extra relationship systems.
